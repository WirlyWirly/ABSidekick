...
ℹ️ When you first open the Audiobookshelf web interface, abs will redirected you slightly by adding `/audiobookshelf/` to the URL. This redirect makes it so that **ABSidekick** does not run when you first load the page, becuase it will only run when the URL contains `/audiobookshelf/` without abs needing to redirect you to it.. To fix this, you can simply add `/audiobookshelf/` to your browser bookmark, that way whenever you open the bookmark, abs doesn't need to redirect you because you are already opening the correct URL.
